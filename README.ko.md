...
AI 코딩 에이전트의 남은 쿼터, 로컬 토큰 사용량, API 단가 기준 예상 비용을 한곳에서 확인하는 개인정보 보호 중심의 macOS 메뉴바 앱입니다.
...
| 영역 | 제공 기능 |
| --- | --- |
| 쿼터 | 남은 비율, 초기화 시각, 제공자 상태 및 모델별 한도 확인 |
| 토큰과 비용 | 로컬에서 관측한 토큰과 API 단가 기준 예상 비용을 달러 또는 원화로 표시 |
| 메뉴바 | 아이콘, 가장 낮은 잔여량, 월 예상 비용 또는 선택한 제공자 잔여량 표시 |
...
현재 배포된 미리보기 버전은 `v0.4.1`입니다. macOS 14 이상과 Apple Silicon이 필요합니다.
...
| 제공자 | 계정 쿼터 | 로컬 사용량 및 비용 데이터 |
| --- | --- | --- |
| Codex | 주간 및 모델별 한도 | `~/.codex/sessions`의 최근 세션 토큰을 감지한 모델 단가로 추정 |
...
Codex와 Claude는 기존 CLI 로그인을 재사용해 계정 사용량 API를 조회합니다. Claude 키체인 접근은 설정에서 사용자가 직접 시작하며 인증정보는 메모리에만 캐시합니다. 계정 조회가 실패하면 임의의 값을 만들지 않고 오래된 상태나 사용 불가 상태를 표시하거나, 검증 가능한 로컬 데이터로 대체합니다.
...
비용은 관측한 토큰을 공개 API 단가로 사용했을 때의 추정액입니다. Codex, Claude, Grok, ChatGPT 또는 Claude 구독의 실제 청구액이 아닙니다.
...
- 월 합계는 앱이 관측한 비용 변화로 재구성하므로 앱이 기록한 기간만 포함합니다.
...
| 탭 | 설정 항목 |
| --- | --- |
| 일반 | UI 언어, 제공자, 제공자 연결, 메뉴바 표시, 선택 제공자, 컴팩트 모드, 활동 애니메이션과 감지 시간, 로그인 시 실행, 업데이트 확인 |
...
- 로컬 파싱은 알려진 집계 사용량 필드와 OpenCode 데이터베이스의 집계 열로 제한합니다.
