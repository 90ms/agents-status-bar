...
| 토큰과 비용 | Codex 계정의 오늘·이번 달·누적 토큰과 제공자별 로컬 사용량을 확인하고, API 환산 참고 비용을 달러 또는 원화로 표시 |
...
| --- | --- | --- |
| Codex | 주간·모델별 한도와 사용 가능한 한도 초기화 충전권 및 만료일 | 실험적인 Codex app-server에서 가져온 오늘·이번 달·누적 계정 토큰과 API 환산 참고 비용 |
...
- 설치된 CLI가 실험적인 app-server 메서드를 지원하면 Codex 계정의 오늘·이번 달·누적 토큰을 로컬 세션 파일과 독립적으로 가져옵니다.
...
Codex 계정 사용량은 정확한 API 비용 계산에 필요한 과거 모델, 입력·출력, 캐시 및 추론 토큰 구분 없이 총 토큰만 제공합니다. 따라서 오늘·이번 달·누적 비용은 거친 API 환산 참고값이며, 이 영역에서는 토큰 합계가 기준 데이터입니다.
