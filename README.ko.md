...
<p align="center">
  <img src="docs/agents-status-bar.png" width="400" alt="코딩 에이전트 사용량을 보여주는 Agents Status Bar 메뉴" />
</p>
...
- 아이콘만, 가장 낮은 잔여량, 월 예상 비용 또는 선택 제공자 잔여량 중 메뉴바 표시 선택
- 로컬 세션 파일이 갱신되는 동안 메뉴바에 활동 애니메이션 아이콘과 제공자 상태 표시
- 핵심 쿼터를 유지하고 부가 정보 행을 숨기는 컴팩트 팝오버
- 6시간 주기의 GitHub Releases 업데이트 확인, 검증 캐시 및 안정 릴리스 바로가기
- Codex, Claude Code, Grok, Gemini CLI 및 OpenCode 지원
- 1분 주기 자동 새로고침 및 수동 새로고침
- 로그인 시 자동 실행 설정
- 제공자별 활성화 및 비활성화 설정
- 시스템 설정, 한국어 또는 English 중 직접 선택할 수 있는 UI 언어
- 일반, 알림, 사용량 및 개인정보로 구분된 탭형 설정 화면
- 프롬프트, 응답, 인증정보, 제공자 상세 내용 및 파일 경로를 제외한 복사 가능한 진단 보고서
...
| Gemini CLI | 로컬에서 확인할 수 없음 | 최근 세션 토큰 | `~/.gemini/tmp/*/chats` |
| OpenCode | 로컬에서 확인할 수 없음 | 전체 집계 토큰 및 기록 비용 | `~/.local/share/opencode/opencode*.db`의 집계 열 |

각 CLI의 파일 형식과 사용량 API는 공개 호환성 규격이 아니므로 변경될 수 있습니다. 계정 조회가 실패하면 임의의 값을 만들지 않고 확인 가능한 로컬 데이터로 대체합니다.

활동 중인 세션은 3초마다 알려진 사용량 파일의 변경 시각만 확인해 감지합니다. 마지막 쓰기 이후 10초, 15초 또는 30초 동안 활동 상태를 유지하도록 설정할 수 있습니다. 활동 중에는 macOS 메뉴바 레이블이 고정 폭의 파형 프레임 두 개를 1초마다 교대합니다. 연속 메뉴바 렌더링을 피하면서 활동 상태를 확인할 수 있습니다. 이는 제공자가 실제 응답을 생성하고 있음을 보장하는 값이 아니라 로컬 활동을 나타내는 휴리스틱입니다. 애니메이션은 끌 수 있으며 macOS의 동작 줄이기 사용 시 정적인 활동 표시로 대체됩니다.

Codex 계정 응답은 자동 갱신 중 최대 1분 동안 캐시합니다. 쿼터 초기화 시각이 지나면 이전 캐시를 즉시 무효화하며, 수동 새로고침 버튼은 제공자 캐시를 우회하므로 충전된 사용량이 기존처럼 최대 5분 늦게 표시되지 않습니다.
...
- Gemini CLI는 가장 최근에 로컬 기록된 토큰 메타데이터만 사용하며 아직 비용을 계산하지 않습니다.
- OpenCode는 제공자별 모델을 다시 계산하지 않고 로컬 세션 데이터베이스에 이미 집계된 비용을 표시합니다.
...
앱은 GitHub Releases에서 새 버전을 자동으로 확인하지만 다운로드와 설치는 직접 진행합니다. 따라서 유료 Apple Developer Program 멤버십 없이도 ad-hoc 서명 빌드를 사용할 수 있습니다.

## 설치

`v0.4.1` 미리보기는 macOS 14 이상과 Apple Silicon을 지원합니다.
...
[Releases](https://github.com/90ms/agents-status-bar/releases)에서 `AgentsStatusBar-0.4.1.zip`을 내려받아 압축을 풀고 `Agents Status Bar.app`을 `/Applications`로 옮깁니다.
...
- 로컬 파싱은 에이전트 세션 디렉터리의 알려진 사용량 필드와 OpenCode 데이터베이스 집계 열로 제한됩니다.
- 활동 감지는 파일 변경 메타데이터만 확인하며 프롬프트나 응답을 열지 않습니다.
...
- 진단 정보에는 정제된 앱·OS 정보와 집계된 제공자 상태, 쿼터, 모델 및 토큰 필드만 포함됩니다.
