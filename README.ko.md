...
Codex와 Claude는 기존 CLI 로그인을 재사용해 계정 사용량 API를 조회합니다. 계정 조회가 실패하면 임의의 값을 만들지 않고 오래된 상태나 사용 불가 상태를 표시하거나, 검증 가능한 로컬 데이터로 대체합니다.
...
| 탭 | 설정 항목 |
| --- | --- |
| 일반 | UI 언어, 제공자, 메뉴바 표시, 선택 제공자, 컴팩트 모드, 활동 애니메이션과 감지 시간, 로그인 시 실행, 업데이트 확인 |
