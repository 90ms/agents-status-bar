...
- 로컬 세션 파일이 갱신되는 동안 메뉴바에 회전형 활동 아이콘과 제공자 상태 표시
...
활동 중인 세션은 3초마다 알려진 사용량 파일의 변경 시각만 확인해 감지합니다. 마지막 쓰기 이후 10초, 15초 또는 30초 동안 활동 상태를 유지하도록 설정할 수 있습니다. 활동 중에는 macOS 메뉴바 레이블 내부의 아이콘이 레이블 폭을 바꾸지 않고 회전합니다. 이는 제공자가 실제 응답을 생성하고 있음을 보장하는 값이 아니라 로컬 활동을 나타내는 휴리스틱입니다. 애니메이션은 끌 수 있으며 macOS의 동작 줄이기 사용 시 정적인 활동 표시로 대체됩니다.
